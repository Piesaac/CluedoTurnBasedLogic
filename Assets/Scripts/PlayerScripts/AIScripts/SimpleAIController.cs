...
public class SimpleAIController : NetworkBehaviour
{
    ...
    private Character characterIdentity;
    ...
    void Start()
    {
        ...
        characterIdentity = GetComponent<Character>();
    }
    ...
    void Update()
    {
        ...
        if (!characterIdentity.isRobot.Value) return;

        // 3. Check the ID assignment
        if (characterIdentity.botID.Value == -1)
        {
        ...
        bool isMyTurn = (characterIdentity.botID.Value == tm.whosPlaying.Value);
    ...
    IEnumerator AIRoutine(TurnManager tm)
    {
        ...
        Debug.Log($"<color=yellow>[AI BRAIN] {characterIdentity.charName} (ID {characterIdentity.botID.Value}) is starting its turn.</color>");
